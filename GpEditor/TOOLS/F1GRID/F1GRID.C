...
** Date   : 18/01/95
**
** $Header:   L:/gpeditor/tools/f1grid/vcs/f1grid.c__   1.2   31 Jan 1995 21:53:08   tk  $
**
** 
*/

#define VERSION     "V1.2 31/Jan/95"
...
main(
     ...
     (void) memset(nums, 0, sizeof(nums));
     ...
     sg   = malloc(sizeof(*sg));
     buff = malloc(MAX_LINE_LEN);

     if (sg == NULL || buff == NULL) {
          (void) printf("f1grid: Not enough free memory.\n");
          return 1;
     }
     ...
     if (!u_flag) {
          ...
          /*
          ** Setup qualifying session so it expires almost immediately (10min).
          */
          sg->current_session_time_ms = (sg->qualifying_period_conf * 60U);
          sg->current_session_time_ms -= 10U * 60U;
          ...
          for (i = 0; i < NUM_ELEMENTS(sg->fastest_lap_time); i++) {
               if (sg->car_numbers[i] != 0) {
                    for (j = 0; j < NUM_ELEMENTS(nums); j++) {
                         ...
                         if (n == nums[j]) {
                              sg->fastest_lap_time[n - 1]     = j + 1L;
                              sg->qualifying_grid_time[n - 1] = j + 1L;
                              break;
                         }

                         for (x = 0; x < NUM_ELEMENTS(sg->per_car); x++) {
                              if ((sg->per_car[x].car_number & 0x3f) == n) {
                                   sg->per_car[x].best_lap_time = j + 1L;
                              }
                         }
                    }
               }
...
Usage(
     ...
     (void) printf( "Now using any ASCII editor (e.g., Windows NOTEPAD) edit the .TXT file and\n"
